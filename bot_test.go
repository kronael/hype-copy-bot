...
func TestFillProcessingThreshold(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			initialTrades := bot.paperTrader.TotalTrades
			err := bot.processFill(tt.fill)

			if err != nil {
				t.Errorf("processFill() error = %v", err)
			}

			tradesAdded := bot.paperTrader.TotalTrades - initialTrades
	...
	if bot.paperTrader.TotalTrades != totalExpectedTrades {
		t.Errorf("Total trades = %d, want %d", bot.paperTrader.TotalTrades, totalExpectedTrades)
	}
}

func TestHyperliquidClientAPI(t *testing.T) {
	// Create mock server
	mockFills := []*Fill{
		{
			Coin:      "BTC",
...
func TestBotDuplicateFillHandling(t *testing.T) {
	...
	err1 := bot.processFill(fill)
	err2 := bot.processFill(fill)

	if err1 != nil || err2 != nil {
		t.Errorf("processFill() errors: %v, %v", err1, err2)
	}

	// Should only process once due to hash tracking
	if bot.paperTrader.TotalTrades != 1 {
		t.Errorf("Duplicate fill processed: trades = %d, want 1", bot.paperTrader.TotalTrades)
	}
...
func TestRealWorldTradingScenario(t *testing.T) {
	...
	if pt.TotalTrades != len(fills) {
		t.Errorf("Total trades = %d, want %d", pt.TotalTrades, len(fills))
	}
...
func TestErrorHandling(t *testing.T) {
	...
	for i, fill := range malformedFills {
		err := bot.processFill(fill)
		if err != nil {
			t.Errorf("processFill(%d) should handle malformed data gracefully, got error: %v", i, err)
		}
	}
