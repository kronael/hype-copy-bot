...
func TestWhaleScenario(t *testing.T) {
	...
	expectedTrades := 5
	if pt.TotalTrades != expectedTrades {
		t.Errorf("Total trades = %d, want %d", pt.TotalTrades, expectedTrades)
	}
}
...
func TestSmallTradeFiltering(t *testing.T) {
	...
	err := bot.processFill(smallFill)

	if err != nil {
		t.Errorf("processFill returned error: %v", err)
	}

	// Should have 0 trades due to threshold
	if bot.paperTrader.TotalTrades != 0 {
		t.Errorf("Small trade was not filtered: got %d trades, want 0", bot.paperTrader.TotalTrades)
	}

	// Large trade - should pass through
	largeFill := createTestFill("ETH", "B", 1.0, 4000.0, "0.0", time.Now().Unix())
	err = bot.processFill(largeFill)
	...
	if bot.paperTrader.TotalTrades != 1 {
		t.Errorf("Large trade was filtered: got %d trades, want 1", bot.paperTrader.TotalTrades)
	}
}
...
func TestZeroAndNegativeSizes(t *testing.T) {
	...
	if pt.TotalTrades != 1 {
		t.Errorf("Zero size trade not recorded: trades = %d, want 1", pt.TotalTrades)
	}
}
...
func TestHighFrequencyTrading(t *testing.T) {
	...
	if pt.TotalTrades != 100 {
		t.Errorf("High frequency test: trades = %d, want 100", pt.TotalTrades)
	}
