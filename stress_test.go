...
func TestConcurrentPaperTrading(t *testing.T) {
	...
	if pt.GetTotalTrades() != expectedTotalTrades {
		t.Errorf("Concurrent test: total trades = %d, want %d", pt.GetTotalTrades(), expectedTotalTrades)
	}
...
func TestMemoryLeakPrevention(t *testing.T) {
	...
	if pt.GetTotalTrades() != numTrades {
		t.Errorf("Memory test: trades = %d, want %d", pt.GetTotalTrades(), numTrades)
	}
}
...
func TestRapidPositionFlips(t *testing.T) {
	...
	for i := 0; i < 50; i++ { // Reduce iterations
...
func TestTimeOrderingEdgeCases(t *testing.T) {
	...
	if pt.GetTotalTrades() != len(fills) {
		t.Errorf("Time ordering test: trades = %d, want %d", pt.GetTotalTrades(), len(fills))
	}
